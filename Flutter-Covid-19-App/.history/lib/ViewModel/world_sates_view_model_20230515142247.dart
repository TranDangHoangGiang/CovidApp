...
class Autogenerateddata {
  ...
  Future<Locations> fetcity( String city) async {
    ...
    if (response.statusCode == 200) {
      var data = jsonDecode(response.body);
      var filteredLocations = data['locations'].firstWhere((location) => location['name'] == city);
        }
...
Future<Locations> fetchCityRecords(String city) async {
  final response = await http.get(Uri.parse('https://static.pipezero.com/covid/data.json'));

  if (response.statusCode == 200) {
    var data = jsonDecode(response.body);
    var filteredLocations = data['locations'].firstWhere((location) => location['name'] == city, orElse: () => null);
